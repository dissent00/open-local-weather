...
| `day_uv_index.json` | `day_uv_index` | 10 |
...
| `llm_user_prompt.json` | `build_user_prompt` | 12 |
...
| `weekly_review.json` | `build_weekly_review` | 25 |
...
| `code_blend.json` | `code_blend_prediction` / `code_blend_predictions` | The record turned into a forecast by arithmetic — ROADMAP item 173. Rain votes weighted by rolling-30 hit rate above 50, the probability the weighted wet share, Day+0 high/low corrected by the rolling-10 error. Three cases land exactly on .5, where Python's half-to-even and Dart's `.round()` differ; a model short of the check threshold or at a coin flip must be absent, not zero-weighted. |
...
| `wind_timeline.json` | `describe_wind_timeline` | 8 |
...
| **A vector that reads the live record** | `run_row.json` read its committed row from `data/log/` until 2026-09-18, when the pipeline's own morning commit re-saved that row with a null key for a field added after it was written and turned CI red on a roadmap-only push. The record is re-saved by every verification pass that touches it. A vector's input is a frozen copy under `spec/fixtures/`. |
