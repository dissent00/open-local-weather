...
# calendar that stopped at Day+3 would leave the model deriving the dates it
# actually writes about, which is the whole failure this exists to remove.
CALENDAR_DAYS = 7


def forward_calendar(today: date, days: int = CALENDAR_DAYS) -> list[dict]:
    ...
    and that is arithmetic, which in this project lives in code. So it is done
    here and handed over finished, exactly as every other computed value is.

    `lead_time_days` is included because the rest of the prompt speaks in
    leads — Day+0, Day+3, Day+7 — and a reader of the block should not have to
    count rows to line it up with the predictions above it.
    """
    return [
        {
            "lead_time_days": lead,
            "date": add_days(today, lead).isoformat(),
            "day_name": weekday_name(add_days(today, lead)),
        }
        for lead in range(days + 1)
    ]
