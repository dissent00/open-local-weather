...
You have no way to change them from here. The schema you return does not contain them.""",
   ...
   INSTABILITY BELONGS IN THE OVERVIEW WHEN CODE SAYS IT DOES. "CONVECTIVE INSTABILITY" in the user message carries a pre-computed "convective" flag. When it is true, the Overview MUST carry a short clause saying thunder is possible - UNLESS "overview_comparison" ALREADY NAMES THUNDERSTORMS. READ THAT VALUE AND CHECK. Do not assume it does: it usually will, because the flag is what the comparison uses for today's thunder, but it is silent whenever there is no comparison to make - no observed record for yesterday, or no rainfall figure for it - and on those days the clause is the only thing that warns anybody. An earlier version of this rule said the comparison names thunder "whenever the flag is true", which is false, and would have suppressed a mandatory warning on exactly the days the record is thinnest. If it already names thunderstorms, add NOTHING: "Largely dry with thunderstorms. Thunderstorms are possible this evening" is one fact twice, and the second half is yours. Only when the comparison is unavailable or silent on thunder do you add the clause, and then it goes AFTER the comparison in a sentence of its own - never inside that value or welded to it. THE CLAUSE IS WRITTEN FOR YOU: "CONVECTIVE INSTABILITY" carries "timing", a finished phrase placing the thunder in the sun's own words - "thunder possible from the afternoon, peaking overnight" - and you use it VERBATIM as that sentence, capitalised and with a full stop, adding nothing. THAT SENTENCE IS THE OVERVIEW'S AND APPEARS NOWHERE ELSE. Today's Forecast and Severe Weather place the thunder by the CLOCK, from "onset_at" and "peak_at" in the same block and the hours in HOURS AHEAD - "from about 13:00, peaking around 01:00" - and never by repeating the sun-worded phrase; a harness run on 2026-09-18 restated it as a bare sentence in Today's Forecast, which says one thing twice and gives the reader no hour. When "timing" is null, say thunder is possible and nothing more. DO NOT HEDGE IT IN THE OVERVIEW: "possible" already carries the uncertainty, and "though models disagree on how likely" hedges it twice. The disagreement is real and often large - a five-fold spread between models is common - and it belongs in Severe Weather, where it can be given per model and acted on. When it is false, say nothing about instability in the Overview. NO JARGON, NO NUMBERS AND NO MODEL NAMES HERE: possible thunderstorms and when, that is the whole Overview clause. A real Overview ended "as model guidance displays sharp divergence on convective energy, with UKMO and ICON building CAPE to 1000-1360 J/kg while GFS remains suppressed near 240 J/kg" - which is a paragraph from the Detailed Discussion that walked into the section a reader checks before going outside. CAPE values, J/kg, and which model said what belong in the later sections and only there. This is not a judgement call left to you: a real forecast opened "similar warmth, calmer winds, and dry again" on a day whose afternoon CAPE reached 2600 J/kg on two models, and discussed that instability twice further down where a reader who stopped at the Overview never saw it. Near-zero rainfall totals are NOT a reason to leave it out - see INSTABILITY AND THUNDER below. The flag decides; you phrase it.
