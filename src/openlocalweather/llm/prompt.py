...
2. STEP 2: SYNTHESIZE TODAY'S NARRATIVE (GitHub Pages & Email Body)
   ...
   EVERY VARIABLE YOU NAME MUST BE IN YOUR INPUT. "Warm and humid through the morning" is a real opening and humidity is not fetched, not forecast and not in this prompt anywhere - it was invented because it sounded like weather. TWO DIFFERENT RULES LIVE HERE, and they were one sentence with a false reason attached. FIRST, A QUANTITY THAT IS NOT IN THE DATA DOES NOT EXIST FOR THIS FORECAST: no humidity, no "feels like". Nothing fetches them, and reporting one is worse than omitting it because a reader cannot tell an invented number from a measured one. SECOND, DEW POINT, VISIBILITY AND CLOUD BASE ARE PRESENT AND ARE STILL NOT YOURS TO REPORT. They are in "airport_metar", twice over — as fields and again inside "rawOb" — so do not go looking for a justification that they are absent, because they are not. They are withheld for a different reason: A METAR IS A POINT OBSERVATION AT ONE AIRPORT AT ONE HOUR, ALREADY HOURS OLD, and these sections are about the day ahead. "Visibility over 10 km" in a forecast narrates what is already over. You may reason FROM them - dew point is the moisture in "high CAPE with modest moisture", and a cumulonimbus group is a storm somebody saw - but do not print the figures. This is written out because the old rule claimed all four were absent, which was false for three of them, and readers who followed the stated reason rather than the list reported them, correctly and twice.
