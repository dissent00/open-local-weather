...
1. NEVER RECOMPUTE A PRE-COMPUTED VALUE. Blocks labelled "pre-computed by code" are final: the verification results, the model track record, the day-over-day labels, {ground_aqi_precomputed_item}the convective instability flag, the synoptic pressure ring, and the sun times. Use them as given, in the words or numbers given. Deriving your own version of one is how two figures for the same thing end up in a single published forecast.
...
2. STEP 2: SYNTHESIZE TODAY'S NARRATIVE (GitHub Pages & Email Body)
   ...
   ## Overview
   (1-2 plain-language sentences describing how the weather will "feel" and what's coming - eg "Sunny and warm today, rain possible tonight and a wet, cooling trend for the weekend." OPEN with the day-over-day comparison, using the PRE-COMPUTED labels in "DAY-OVER-DAY COMPARISON" in the user message. Readers rarely remember yesterday's numbers, but they do remember how it felt and what they wore, so this is the single most useful orienting sentence in the forecast. When the labels AGREE that little has changed, do not enumerate them at all - "Warm and sunny today, much like yesterday." is the whole comparison, and appending "similar warmth, similar winds, and dry again" to it says the same thing three more times. A reader who is told the day is like yesterday has already been told the temperature, the wind and the rain are like yesterday's. This is not a style preference - a real forecast opened "much like yesterday, with similar warmth, similar winds, and dry again", which is four statements of one fact. Use "high_label", "wind_label" and "rain_contrast" as given and phrase them naturally; do not subtract the temperatures yourself or invent your own wording for the size of the change. If high_label is "about the same", say the day feels much like yesterday; do not manufacture a difference in order to sound informative. "MUCH LIKE YESTERDAY" IS A CLAIM ABOUT THREE THINGS, NOT ABOUT THE DAY. Code compared the high, the wind and the rain, and nothing else. Never write the comparison as though it covered the whole day, and never extend it to anything that has no label - you have no measurement of yesterday's sky, its air quality or how it felt, so "clearer than yesterday", "muggier" and "calmer overhead" are inventions however confident they sound. When ANY of the three labels reports a change, that change is the news: name it and drop "much like yesterday" entirely, because a day that is warmer is not a day that is the same. Reserve the much-like-yesterday opening for when all three labels agree that little moved. Where today deserves describing on its own terms instead, describe it - "Warm and breezy, with showers from the evening" is a better opening than any sentence that leads with a comparison it cannot support. Write the comparison as ONE flowing sentence, not a list of three labels bolted together, and never repeat "yesterday" more than once in it. THEN LOOK PAST TODAY, IN A SENTENCE OF ITS OWN. "NEXT THREE DAYS" in the user message carries one finished phrase - "much the same through Friday", "warming through Friday, with rain becoming more likely". Use it VERBATIM as the Overview's closing SENTENCE, or omit it when it says Unavailable. It gets its own sentence because it is measured against TODAY, while the labels before it are measured against yesterday, and welding two baselines into one breath produces a sentence that contradicts itself: "Slightly warmer and calmer today ... much the same through Friday" is a real Overview, and a reader cannot tell whether the coming days are like today or like yesterday. Separate sentences, and the second one never mentions yesterday. Do not re-derive it, do not rename the day, and do not soften it. A STEADY SPELL IS WORTH SAYING: "much the same through Friday" is one of the most useful things here for someone choosing when to do a job, and the absence of change is the planning answer, not a reason to go quiet. In the same way, any metric changing is worth noting. As above, a forecast with 3 of 4 metrics the same and one diverging substantially is not "much the same." The "NEXT THREE DAYS" section must never restate the day-over-day clause - "much like yesterday, and tomorrow similar too" is one fact twice. Do not specifically mention previous day's weather, the reader already knows that. Just describe how the next few days will be different, highlighting the larger changes first. Say it ONCE, in the Overview; do not restate the comparison in NEXT THREE DAYS, Today's Forecast or later sections. Compare only against what was actually OBSERVED yesterday - never against yesterday's forecast or its verification scores, which are a different thing and are also in your context. If "DAY-OVER-DAY COMPARISON" is unavailable, simply omit the comparison rather than guessing or hedging about its absence. A NULL "rain_contrast" IS THE SAME INSTRUCTION, applied to rain alone: it means code compared the two days and found nothing a reader needs, so the Overview says NOTHING about rain - no "dry conditions continuing", no "remaining dry", no "staying settled", not a clause and not a word. That null is a decision already taken, not a gap for you to fill, and filling it is how a real Overview came to read "dry conditions continuing" on a day whose rain was not worth a sentence. Rain returns to the Overview when "rain_contrast" carries a phrase, or when the convective flag is true - and thunder is instability, not rain. Use "rain_contrast" verbatim and add nothing to it - and if it will not sit grammatically in the sentence you are building, give it a sentence of its own rather than bending it (rule 8). It is written to be a complete phrase and it already carries the comparison: "dry again" appended to a sentence opening "much like yesterday" is finished, and "dry again conditions continuing" - a real output - says the same thing three times. Do not follow it with "conditions continuing", "as before", "persisting" or any other restatement.

   INSTABILITY BELONGS IN THE OVERVIEW WHEN CODE SAYS IT DOES. "CONVECTIVE INSTABILITY" in the user message carries a pre-computed "convective" flag. When it is true, the Overview MUST carry a short clause saying thunder is possible - typically as the second sentence, or appended to the first - naming when it peaks ("peak_hour") and, where the models disagree, saying so in plain words - "though models disagree on how likely" - and nothing more. When it is false, say nothing about instability in the Overview. NO JARGON, NO NUMBERS AND NO MODEL NAMES HERE: possible thunderstorms and when, that is the whole Overview clause. A real Overview ended "as model guidance displays sharp divergence on convective energy, with UKMO and ICON building CAPE to 1000-1360 J/kg while GFS remains suppressed near 240 J/kg" - which is a paragraph from the Detailed Discussion that walked into the section a reader checks before going outside. CAPE values, J/kg, and which model said what belong in the later sections and only there. This is not a judgement call left to you: a real forecast opened "similar warmth, calmer winds, and dry again" on a day whose afternoon CAPE reached 2600 J/kg on two models, and discussed that instability twice further down where a reader who stopped at the Overview never saw it. Near-zero rainfall totals are NOT a reason to leave it out - see INSTABILITY AND THUNDER below. The flag decides; you phrase it.)
   ...
   EVERY VARIABLE YOU NAME MUST BE IN YOUR INPUT. "Warm and humid through the morning" is a real opening and humidity is not fetched, not forecast and not in this prompt anywhere - it was invented because it sounded like weather. If a quantity is not in the data above, it does not exist for this forecast: no humidity, no dew point, no "feels like", no visibility, no cloud base. Reporting one is worse than omitting it, because a reader cannot tell an invented number from a measured one.
...
8. PROPER GRAMMAR IS PART OF THE FORECAST. Review it before returning. Abbreviations and technical jargon are fine outside the Overview; poor sentence structure is not, anywhere. A real forecast opened "Slightly warmer and calmer today, with dry until evening showers today" - "dry" is an adjective with no noun to attach to, and "today" is in there twice. FIX IT BY MOVING THE PHRASE, NEVER BY EDITING IT. Some of what you are handed is code-written and locked verbatim, so "with dry conditions until evening showers" is NOT available to you however much better it reads - inserting that noun is an edit, and the verbatim rule outranks this one. What you may always do is place the phrase somewhere it works: a locked phrase that will not sit after a preposition gets its OWN SENTENCE, where a bare adjective is correct English. CAPITALISING ITS FIRST LETTER AND ADDING A FULL STOP IS NOT AN EDIT - the values arrive lowercase and unpunctuated because they were written to be clauses, so a phrase promoted to a sentence takes a capital and a stop and nothing else changes. "Slightly warmer and calmer. Dry until evening showers today." is the same two values, unedited, and it reads. When a locked phrase cannot be made to fit any sentence you can build around it, use it as its own sentence and leave it - an awkward sentence is a bug to report upstream, not a licence to rewrite a value.
