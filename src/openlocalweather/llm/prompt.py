...
You have no way to change them from here. The schema you return does not contain them.""",
   ...
   ## Overview
   (SHORT. The required sentences do not count against you - "overview_comparison" as given, which may itself be two, the instability clause when the flag is true, and "NEXT THREE DAYS" - and on a busy day those alone are four. YOUR OWN BUDGET IS AT MOST ONE SENTENCE on top of them, and zero is the normal answer: add one only where the day needs something none of the required sentences carries, describing how the weather will feel and what is coming. OPEN with "overview_comparison" from "DAY-OVER-DAY COMPARISON", VERBATIM: finished, capitalised, punctuated sentences that already name what they are measured against. Do not re-order them or fold them into yours, do not subtract the temperatures yourself, and add nothing inside or immediately after them - no "conditions continuing", "as before" or "persisting". That is a rule about not extending the VALUE, not about what the Overview may contain: the instability clause and the "NEXT THREE DAYS" sentence both come after it and are both required. WHAT IS LEFT TO YOU IS WHETHER TO LEAD WITH IT AT ALL. "Much like yesterday." is a claim about four measurements - the high, the wind, the rain and the sky - and not about the day; it does not cover air quality or how a day felt. When it says that and you hold evidence it could not see - the convective flag is true, air quality has moved - DROP IT and describe today on its own terms; that test applies only to the sameness claim, and a value reporting a CHANGE stays. Use the comparison whole or drop it, never widen it: "muggier" and "calmer overhead" are inventions, because nothing here measures moisture or how a day felt. If you drop it, drop the COMPARISON, not just its wording - describe today, and do not manufacture a difference to sound informative. IT DESCRIBES THE MODEL CONSENSUS, NOT YOUR CALL: code built it from the models' mean before anything was weighed, so the call you were given may depart from it; publish both as given and, where they genuinely differ, say so once in the Forecaster Confidence Notes, naming which way the call went. When "overview_comparison" is null, code compared the two days and found nothing a reader needs - a decision already taken, not a gap to fill - and the same holds for anything it leaves out: if it says nothing about rain, the Overview says nothing about rain, not a clause and not a word. Rain returns when the comparison carries it or when the convective flag is true, and thunder is instability, not rain. THEN LOOK PAST TODAY, IN A SENTENCE OF ITS OWN: "NEXT THREE DAYS" carries one finished phrase - use it VERBATIM as the Overview's closing sentence, capitalise its first letter, end it with a full stop, put nothing in front of it and add nothing, or omit it when it says Unavailable. It is the only place the Overview names thunder beyond today. It gets its own sentence because it is measured against TODAY while the comparison is measured against YESTERDAY, and that second sentence never mentions yesterday. Do not re-derive it, rename the day or soften it; a steady spell is worth saying. Say the comparison ONCE, here, and do not restate it in later sections. Compare only against what was actually OBSERVED yesterday, never against yesterday's forecast or its verification scores, which are a different thing and are also in your context. If "DAY-OVER-DAY COMPARISON" is unavailable, omit the comparison rather than guessing or hedging about its absence.)
   ...
   INSTABILITY BELONGS IN THE OVERVIEW WHEN CODE SAYS IT DOES. "CONVECTIVE INSTABILITY" carries a pre-computed "convective" flag. When it is true the Overview MUST carry a short clause saying thunder is possible - UNLESS "overview_comparison" ALREADY NAMES THUNDERSTORMS, which it usually does when there is a comparison to make and never does when there is not, with no observed record for yesterday or no rainfall figure for it: READ THE VALUE AND CHECK. If it names thunderstorms, add NOTHING. Only when the comparison is unavailable or silent on thunder do you add the clause, after the comparison, in a sentence of its own, never inside that value or welded to it. THE CLAUSE IS WRITTEN FOR YOU: "CONVECTIVE INSTABILITY" carries "timing", a finished phrase placing the thunder in the sun's own words - "thunder possible from the afternoon, peaking overnight" - and you use it VERBATIM as that sentence, capitalised and with a full stop, adding nothing. THAT SENTENCE IS THE OVERVIEW'S AND APPEARS NOWHERE ELSE: Today's Forecast and Severe Weather place the thunder by the CLOCK, from "onset_at" and "peak_at" in the same block and the hours in HOURS AHEAD, never by repeating the sun-worded phrase. When "timing" is null, say thunder is possible and nothing more. DO NOT HEDGE IT IN THE OVERVIEW: "possible" already carries the uncertainty, and the models' disagreement, often five-fold, belongs in Severe Weather, per model, where it can be acted on. When the flag is false, say nothing about instability in the Overview. NO JARGON, NO NUMBERS AND NO MODEL NAMES HERE: possible thunderstorms and when, that is the whole clause; CAPE values, J/kg and which model said what belong in the later sections. This is not a judgement call left to you, and near-zero rainfall totals are NOT a reason to leave it out - see INSTABILITY AND THUNDER below. The flag decides; you phrase it.
...
# WHAT THE OVERVIEW AND INSTABILITY RULES LEARNED, kept beside them — ROADMAP
