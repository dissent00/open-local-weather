...
Use each finding at the confidence it states and do not upgrade it - "provisional" is not "established", and a finding is a description of the record so far, never a guarantee about today. Being straightforwardly honest that the record is still thin is correct and expected here, not a failure; this system's value comes from its accuracy claims being checkable, which requires never claiming more than the record holds.''',
    'lead_time': '''LEAD-TIME AWARENESS: A model's Day+0 skill and its Day+3/Day+7 skill can differ substantially - some models hold up better at range than others. When you call Day+3 and Day+7, consult that lead time's OWN track record, not the Day+0 numbers - a model excellent at Day+0 is not automatically trustworthy at Day+7.''',
...
/// The three Forecaster Confidence Notes rules — weighting, sufficiency,
/// voice — were moved here from the judgment prompt by upstream item 162.
/// The account of why, and why the weighting one is obeyable here at all,
/// is in the comment above `build_narrative_prompt` in prompt.py, which
/// this string mirrors character for character.
...
String buildUserPrompt({
  ...
  /// Which kind of nothing, when [groundAqiLastKnown] is null — upstream
  /// item 163. Composed by `lastKnownAbsence` and passed in, so this file
  /// asserts no cause it has not been told.
  String? groundAqiLastKnownAbsence,

  /// The day's peak UV, for the day the horizon points at — upstream item
  /// 161. The forecaster used to be asked for this and was copying one
  /// model's number; what it could not do was pick the DAY, because at
...
${groundAqiLastKnown == null ? (groundAqiLastKnownAbsence ?? 'Unavailable — and the reason was not supplied to this prompt, which is a wiring gap rather than a fact about the stations. Take the air quality from the model guidance and say nothing about the ground sensors either way.') : promptJson(groundAqiLastKnown)}'''
      : '';
...
${peakUvIndex != null ? promptJson(peakUvIndex) : 'Unavailable — no configured source served a UV index for that day. Say nothing about UV rather than reaching for the raw arrays.'}
